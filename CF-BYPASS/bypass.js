const EventEmitter = require('events'); // downloaded by Joker0day
const emitter = new EventEmitter(); // downloaded by Joker0day
emitter.setMaxListeners(Number.POSITIVE_INFINITY); // downloaded by Joker0day
var fs = require('fs'); // downloaded by Joker0day
let net = require('net'); // downloaded by Joker0day
var proxies = fs.readFileSync(process.argv[4], 'utf-8').replace(/\r/g, '').split('\n'); // downloaded by Joker0day

process.on('uncaughtException', function (err) { // downloaded by Joker0day
//  console.log(err);
});

process.on('unhandledRejection', function (err) { // downloaded by Joker0day
//  console.log(err);
});
